...
istream& operator>>(istream& i, Student& s)
{
    ...
    for (int i = 0; i < s.nr_opt; i++) {
        ...
        while (ct == 0) {
            try {
                ...
                cout << "caracterul introdus nu este valid, reincercati!" << endl;
            }
        }
    }
    ...
    while (ct == 0) {
        try {
            ...
            cin >> s.mod_plata;
            if (s.mod_plata == 'c' && s.online == 1)
                throw 1;
            ct = 1;
        }
        catch (int) {
            cout << "imposibil; plata se face numai online sau prin transfer bancar; reincercati " << endl;
        }
    }
    if (s.mod_plata == 'o') {
